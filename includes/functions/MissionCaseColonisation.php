...
function MissionCaseColonisation($FleetRow, &$_FleetCache)
{
    ...
    if ($FleetRow['calcType'] == 1) {
        $AllowedTotalPlanetsWithAstrophysicTech = ceil($_User["tech_expedition"] / 2) + 1;
        ...
        if ($CheckGalaxy['galaxy_id'] <= 0) {
            ...
            if ($_User['id'] != $FleetRow['fleet_owner']) {
                if (empty($_FleetCache['users'][$FleetRow['fleet_owner']])) {
                    ...
                    $AllowedTotalPlanetsWithAstrophysicTech = ceil($_FleetCache['users'][$FleetRow['fleet_owner']]['tech_expedition'] / 2) + 1;
                    $MaxPlanets = $AllowedTotalPlanetsWithAstrophysicTech + $_FleetCache['users'][$FleetRow['fleet_owner']]['additional_planets'];
                }
            } else {
                $MaxPlanets = $AllowedTotalPlanetsWithAstrophysicTech + $_User['additional_planets'];
            }
            ...
            while ($FetchData = $GetUserData->fetch_assoc()) {
                if ($FetchData['Type'] == 1) {
                    ...
                    // TODO - retrieve user tech_expedition to calculate $AllowedTotalPlanetsWithAstrophysicTech also here
